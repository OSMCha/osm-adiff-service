...
const { OSM_CHANGESET_API, CHANGE_STATES, TYPES } = require('./constants');

const OverpassPrimaryUrl = process.env.OverpassPrimaryUrl || 'https://overpass.osmcha.org';
const OverpassSecondaryUrl = process.env.OverpassSecondaryUrl || 'https://overpass-api.de';
...
const queryOverpass = async (changesetId, data, bbox) => {
  const primaryUrl = `${OverpassPrimaryUrl}/api/interpreter?data=${data}&bbox=${bbox}`;
  const secondaryUrl = `${OverpassSecondaryUrl}/api/interpreter?data=${data}&bbox=${bbox}`;
...
const checkLatest = async (max) => {
  const primaryUrl = `${OverpassPrimaryUrl}/api/augmented_diff_status`;
  const secondaryUrl = `${OverpassSecondaryUrl}/api/augmented_diff_status`;
