...
const queryOverpass = async (changesetId, data, bbox) => {
  const { OverpassPrimaryUrl, OverpassSecondaryUrl } = process.env;
  const primaryUrl = `${OverpassPrimaryUrl}/interpreter?data=${data}&bbox=${bbox}`;
  const secondaryUrl = `${OverpassSecondaryUrl}/interpreter?data=${data}&bbox=${bbox}`;
...
const checkLatest = async (max) => {
  const { PrimaryAdiffBaseUrl, SecondaryAdiffBaseUrl } = process.env;
  const primaryUrl = `${PrimaryAdiffBaseUrl}latest`;
  const secondaryUrl = `${SecondaryAdiffBaseUrl}latest`;
